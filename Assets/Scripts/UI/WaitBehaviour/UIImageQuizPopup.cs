...
namespace UI
{
    public class UIImageQuizPopup : WaitBehaviour
    {
        [SerializeField] private Image quizBase;
        [SerializeField] private Image peopleImg;
        [SerializeField] private TextMeshProUGUI titleText;

        [SerializeField] private UIImageQuizButton originOptionButton;
        private List<UIImageQuizButton> optionButtons = new();
        ...
        private List<UIImageQuizButton> GetOption(int count)
        {
            DisableOption();
            ...
            var resultButtons = new List<UIImageQuizButton>();
        ...
        public override IEnumerator Wait(string parameter = "")
        {
            ...
            var quizSelects = quizTexts[2].Split(',');
            ...
            yield return uiSpecialText.Wait($"{DataManager.Instance.PlayerName}의 선택 <#000000>{quizSelects[selectIndex]}");
            ...
            {
                nameText = "후와",
                scriptText = $"{DataManager.Instance.PlayerName}님께서 말씀해주신, {quizSelects[selectIndex]}.",
                waitSeconds = 1f,
            ...
            foreach (var dialog in DataManager.Instance.GetDialogs(resourceName + "_" + quizSelects[selectIndex]).dialogs)
            {
